...
function LP_OPF(dLine::DataFrame, dGen::DataFrame, dNodes::DataFrame, nN::Int, nL::Int, bMVA::Int, solver::String) 
    ...
    if termination_status(m) == OPTIMAL || termination_status(m) == LOCALLY_SOLVED || termination_status(m) == ITERATION_LIMIT
        ...
        # First column: node
        # Second column: value (from variable "P_G", in pu, converted to MVA)
        solGen = DataFrames.DataFrame(bus = (dGen.bus), powerGen = (value.(P_G[dGen.bus]) * bMVA))
        ...
        # First column: node
        # Second column: voltage magnitude (1)
        # Third column: angle value in degrees
        ...
        return m, solGen, solFlows, solVoltage

    # If the optimization problem has no solution, display the error
    else
        return m, 0, 0, 0
