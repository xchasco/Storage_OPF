...
function LP_OPF(dLine::DataFrame, dGen::DataFrame, dNodes::Vector{DataFrame}, nN::Int, nL::Int, bMVA::Int, solver::String, hours::Int, dSolar::DataFrame, dWind::DataFrame, dStorage::DataFrame)
    ...
    # We initialize the previous energy storage state of charge as all zeros (We consider all batteris completedly discharged)
    prev_E_s = zeros(nN)
    ...
    for hour in 1:hours
        ...
        @constraint(m, [i in 1:nN], P_G[i] + (G_Solar[i] + G_Wind[i] - P_Curt[i]) + (P_s_d[i] - P_s_c[i]) - P_Demand[i] == sum(B[i, j] * (θ[i] - θ[j]) for j in 1:nN))
        ...
        if termination_status(m) == OPTIMAL || termination_status(m) == LOCALLY_SOLVED || termination_status(m) == ITERATION_LIMIT
            ...
            # Save the energy storage state of charge for the next iteration
            for i in 1:nN
                prev_E_s[i] = value(E_s[i])
            end
